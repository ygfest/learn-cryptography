...
const HillCipher: React.FC = () => {
  ...
  const processText = (isEncrypt: boolean): void => {
    ...
    let processedTextVector: number[] = multiplyMatrix(
      keyMatrix,
      textVector,
      26
    );
...
const invertMatrixMod = (matrix: number[][], mod: number): number[][] | null => {
...
const HillCipher: React.FC = () => {
  ...
  const processText = (isEncrypt: boolean): void => {
    ...
    if (isEncrypt) {
      const textVector: number[] = text.toUpperCase().split("").map(c => alphabet.indexOf(c));
      if (textVector.includes(-1)) {
        alert("Invalid character in input");
        return;
      }
